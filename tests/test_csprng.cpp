...
TEST(CSPRNGTest, BytesSize) {
    auto buf16 = CSPRNG::bytes(16);
    EXPECT_EQ(buf16.size(), 16u);

    auto buf0 = CSPRNG::bytes(0);
    EXPECT_EQ(buf0.size(), 0u);

    auto buf1024 = CSPRNG::bytes(1024);
    EXPECT_EQ(buf1024.size(), 1024u);
}

TEST(CSPRNGTest, BytesNotAllZero) {
    auto buf = CSPRNG::bytes(64);
    bool all_zero = std::all_of(buf.begin(), buf.end(),
                                [](uint8_t b) { return b == 0; });
    EXPECT_FALSE(all_zero) << "64 random bytes should not all be zero";
}

TEST(CSPRNGTest, BytesUnique) {
    auto a = CSPRNG::bytes(32);
    auto b = CSPRNG::bytes(32);
