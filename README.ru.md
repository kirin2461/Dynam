...
**Версия**: 1.7.0
**Версия CMake**: 1.7.0 (синхронизирована)

### Что нового в 1.7.0 — харденинг в стиле AWG 3.1 + scoped trust для НУЦ + автоустановщик

- **v1.7.0 — защита заголовков в стиле AWG 3.1.** Статичные magic-префиксы кадров (`PH` — port-hopping, `FOG` — fog mesh, `RS` — AEMM-шарды) заменены на сессионные HKDF-SHA256-префиксы (от общего секрета + epoch/seq/block_id), проверяемые за константное время — готовой сигнатуры для DPI больше нет. Новый модуль `ncp_header_protection` (+ `ncp_timer_range`).
- **Рандомизация версии диапазоном (аналог H1–H4).** Байт версии кадра выбирается случайно из диапазона, уникального для установки (`--ver-range <min>-<max>` у `ncp porthop` / `ncp fog`); приёмник принимает весь диапазон — универсальное DPI-правило написать невозможно.
- **Content padding (аналог ContentPaddingAddition).** `--content-padding <min>-<max>` дополняет каждый датаграмму porthop случайным паддингом, ломая статистику кратности 16 байтам.
- **Таймеры-диапазоны.** `--hop-interval 30-120` сэмплируется на каждую эпоху (против ML-классификаторов ТСПУ, ловящих фиксированные константы); примитив `TimerRange` доступен всем модулям.
- **CPS-цепочки (аналог I1–I5).** Новая команда `ncp cps` и опция `--cps "dns:x.ru;wait:20-80;quic;tls:y.ru"` у porthop-клиента: фейковые DNS/QUIC/TLS-пакеты до рукопожатия против активного зондирования.
- **Scoped trust для НУЦ.** Новый модуль `ncp_scoped_trust` + CLI `ncp trust`: корень НУЦ живёт только внутри процесса и применяется ТОЛЬКО к белому списку RU-доменов (`.ru/.su/.рф` + банки/госсервисы); всё остальное — публичные корни. События `custom_root_validation` для ленты GUI; генерация локального CA на установку; автообновление цепочки НУЦ с проверкой SHA-256 + Ed25519-подписи.
- **Автоустановщик сервера (`deploy/`).** `sudo ./install.sh` поднимает Docker Compose: AmneziaWG 3.1 (уникальные параметры на каждую установку) + `ncp reality` + `ncp spa` + `ncp porthop`; генерирует все ключи, настраивает файрвол и выдаёт клиентский комплект (AWG-конфиг + QR, команда SPA knock, параметры porthop/reality). `uninstall.sh` удаляет всё.
...
- ✅ **Тесты**: 786 тестов — 778 пройдено, 8 пропущено (интеграционные тесты I2P требуют живой SAM-мост), 0 упавших.
