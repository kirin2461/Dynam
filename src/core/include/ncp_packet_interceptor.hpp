...
private:
    // Impl is defined as public nested class so platform backends
    // (WFPBackend, NFQUEUEBackend, WinDivertBackend) can inherit from it
    // in the .cpp translation unit.
public:
    class Impl;
private:
    friend class Impl;
    ...
    Config config_;
    std::atomic<bool> initialized_{false};  // R7-ORCH-03: atomic for thread-safe access
...
public:
    // Exposed for platform backend implementations (WFP, NFQUEUE, WinDivert)
    void log_from_backend(const std::string& msg) { log(msg); }

    // Exposed for platform backend implementations that need packet processing
    Stats& backend_stats() { return stats_; }
    Verdict invoke_packet_handler(std::vector<uint8_t>& packet, bool is_outbound) {
        if (packet_handler_) return packet_handler_(packet, is_outbound);
        return default_packet_handler(packet, is_outbound);
    }
private:
