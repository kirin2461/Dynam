...
public:
    ...
    struct LayeredConfig {
        ...
        bool panic_on_debugger = true;
        bool detect_vm = false;
        bool enable_pluggable_transports = false;
        std::string pluggable_transport_type = "obfs4";
    };
    ...
    struct NetworkIsolation {
        ...
        // Kill switch
        // R9-H09: Added timeout to prevent permanent network lockout
    ...
    struct TrafficAnalysisResistance {
        ...
        struct WFDefenseConfig {
            bool enabled = false;
            int burst_size = 10;
            int burst_interval_ms = 100;
            bool padding_enabled = true;
            bool cover_traffic = true;
        };
        WFDefenseConfig wfp_config;

        struct DecoyConfig {
            std::vector<std::string> destinations;
            int max_decoy_hops = 3;
            bool randomize_timing = true;
            std::string decoy_pattern = "random";
        };
        DecoyConfig decoy_config;

        // Decoy routing
        bool enable_decoy_routing = false;
    };
    ...
    struct SecurityAudit {
        ...
        std::vector<std::string> findings;  // detailed findings
...
private:
    ...
    void check_kill_switch_timeout();  // R11-H02: Check and auto-disable if timeout expired
    ...
    void trigger_panic(const std::string& reason);
