 ...
 * - HDD: multi-pass pattern overwrite (effective)
 * - SSD: attempts TRIM/SECURE ERASE via ioctl, falls back to overwrite
 * - CoW FS (btrfs, ZFS): logs warning about ineffective overwrite
 *
 * R9-H10 WARNING: On SSDs with wear leveling, secure deletion is NOT guaranteed.
 * Flash translation layers may remap blocks, leaving old data in spare areas.
 * For true secure deletion on SSDs, use hardware SECURE ERASE command or
