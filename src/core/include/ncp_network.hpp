...
public:
    ...
    struct BypassConfig {
        ...
        uint16_t tcp_window_size = 65535;  // TCP window size for packet construction
    };
...
private:
    ...
    // Active socket for bypass techniques that need setsockopt
    // Set by caller before invoking bypass methods, or created on demand.
#ifdef _WIN32
    SOCKET active_socket_ = INVALID_SOCKET;
#else
