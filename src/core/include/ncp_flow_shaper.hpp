...
struct FlowShaperStats {
    ...
    // R6-06: Use atomic for double members (stored as uint64_t bit pattern)
    std::atomic<uint64_t> current_upload_ratio_bits{0};
    std::atomic<uint64_t> overhead_percent_bits{0};
    ...
    void reset() {
        ...
        current_upload_ratio_bits.store(0);
        overhead_percent_bits.store(0);
    }

    // Helper methods for atomic double access
    double get_current_upload_ratio() const {
        uint64_t bits = current_upload_ratio_bits.load(std::memory_order_relaxed);
        double val;
        std::memcpy(&val, &bits, sizeof(val));
        return val;
    }

    void set_current_upload_ratio(double val) {
        uint64_t bits;
        std::memcpy(&bits, &val, sizeof(val));
        current_upload_ratio_bits.store(bits, std::memory_order_relaxed);
    }

    double get_overhead_percent() const {
        uint64_t bits = overhead_percent_bits.load(std::memory_order_relaxed);
        double val;
        std::memcpy(&val, &bits, sizeof(val));
        return val;
    }

    void set_overhead_percent(double val) {
        uint64_t bits;
        std::memcpy(&bits, &val, sizeof(val));
        overhead_percent_bits.store(bits, std::memory_order_relaxed);
    }
    ...
    FlowShaperStats(const FlowShaperStats& o)
        ...
        , current_upload_ratio_bits(o.current_upload_ratio_bits.load())
        , overhead_percent_bits(o.overhead_percent_bits.load()) {}
};
