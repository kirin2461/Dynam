...
public:
    ...
    struct SpoofConfig {
        ...
        std::vector<std::string> dns_rotation_pool;  // Configurable pool for rotate_dns(); empty = use built-in fallback
        ...
        // R9-H03: Regional DoH fallback servers
        // When primary DoH servers are blocked, use regional alternatives
        bool enable_regional_fallback = true;
        std::vector<std::string> regional_doh_servers = {
            // Russia
            "https://dns.yandex.net/dns-query",
            // China
            "https://doh.pub.com/dns-query",
            // Europe
