...
// Move constructor — std::mutex is not movable, so we lock the source,
// move all non-mutex fields, and leave the new mutex default-constructed.
BurstMorpher::BurstMorpher(BurstMorpher&& other) noexcept
    : config_(std::move(other.config_)),
      stats_(other.stats_),
      cache_(PerturbationCache(other.config_.cache_max_entries)),
      target_db_(),
      burst_tracker_(other.config_.burst_gap_threshold_us),
      latest_burst_(std::move(other.latest_burst_)),
      has_completed_burst_(other.has_completed_burst_) {
    // Note: cache_, target_db_, burst_tracker_ are re-initialized fresh
    // because their internal mutexes cannot be moved. The data inside
    // them is ephemeral (caches are rebuilt at runtime).
}

BurstMorpher& BurstMorpher::operator=(BurstMorpher&& other) noexcept {
    if (this != &other) {
        config_ = std::move(other.config_);
        // stats_ — keep our own atomics, just copy values
        stats_.bursts_observed.store(other.stats_.bursts_observed.load());
        stats_.perturbations_applied.store(other.stats_.perturbations_applied.load());
        stats_.cache_hits.store(other.stats_.cache_hits.load());
        stats_.cache_misses.store(other.stats_.cache_misses.load());
        stats_.fallbacks.store(other.stats_.fallbacks.load());
        stats_.bytes_overhead.store(other.stats_.bytes_overhead.load());
        stats_.bytes_original.store(other.stats_.bytes_original.load());
        latest_burst_ = std::move(other.latest_burst_);
        has_completed_burst_ = other.has_completed_burst_;
        // cache_, target_db_, burst_tracker_ keep their current state
        // (mutexes cannot be moved)
    }
    return *this;
}
...
BurstMorpher::PerturbationResult BurstMorpher::select_perturbation(
    ...
    // R6-07: Snapshot config under lock
    BurstMorpherConfig cfg_snap;
    {
        std::lock_guard<std::mutex> lock(config_mutex_);
        cfg_snap = config_;
    }

    if (!cfg_snap.enabled || payload_size == 0) {
        return {};  // no-op
    }
    ...
    TargetTrafficType target_type = cfg_snap.default_target;
    ...
    } else if (cfg_snap.auto_select_target) {
        target_prof = target_db_.find_nearest(burst);
        if (target_prof) target_type = target_prof->type;
    }

    if (!target_prof) {
        target_prof = target_db_.get(cfg_snap.default_target);
    }
    if (!target_prof) {
    ...
    if (cfg_snap.fallback_to_rule_based || cfg_snap.generate_random_on_miss) {
        stats_.fallbacks.fetch_add(1);
        return generate_fallback_(payload_size, *target_prof, cfg_snap);
    }
...
BurstMorpher::PerturbationResult BurstMorpher::generate_fallback_(
    ...
    const BurstMorpherConfig& cfg_snap) {
    ...
    if (target_size > payload_size) {
        ...
        pre_len = cfg_snap.min_pre_padding;
    }

    // Clamp to limits
    pre_len = (std::max)(pre_len, cfg_snap.min_pre_padding);
    pre_len = (std::min)(pre_len, cfg_snap.max_pre_padding);
    ...
    if (total_orig > 0) {
        ...
        if (current_overhead_pct > cfg_snap.max_overhead_percent) {
            pre_len = cfg_snap.min_pre_padding;  // use minimum
        }
    }
