...
// R9-H12: Named constants for padding sizes (avoid magic numbers)
namespace {
constexpr size_t kAdversarialPaddingMin = 128;
constexpr size_t kAdversarialPaddingMax = 512;
constexpr double kDummyPacketRatio = 0.25;
constexpr double kFlowDummyRatio = 0.35;
}
...
OrchestratorStrategy OrchestratorStrategy::paranoid() {
    ...
    // Maximum adversarial padding (R9-H12: use named constants)
    s.enable_adversarial = true;
    s.adversarial_config = AdversarialConfig::aggressive();
    s.adversarial_config.pre_padding_min = kAdversarialPaddingMin;
    s.adversarial_config.pre_padding_max = kAdversarialPaddingMax;
    s.adversarial_config.dummy_packet_ratio = kDummyPacketRatio;

    // Maximum flow shaping with constant rate
    s.enable_flow_shaping = true;
    s.flow_config = FlowShaperConfig::web_browsing();
    s.flow_config.enable_flow_dummy = true;
    s.flow_config.dummy_ratio = kFlowDummyRatio;

    // Strictest probe resistance
    s.enable_probe_resist = true;
    s.probe_config = ProbeResistConfig::strict();

    // Most compatible mimicry profile
    s.enable_mimicry = true;
    s.mimic_profile = TrafficMimicry::MimicProfile::HTTPS_APPLICATION;

    // Aggressive TLS fingerprint rotation
    s.enable_tls_fingerprint = true;
    s.tls_browser_profile = ncp::BrowserType::CHROME;
    s.tls_rotate_per_connection = true;
    ...
    // ECH fallback enabled for CRITICAL
    s.enable_ech_fallback = true;
...
void ProtocolOrchestrator::derive_dummy_key_from_secret_(
    ...
    // FIX #5: Check salt length to prevent silent truncation
    // libsodium may truncate salts exceeding crypto_auth_hmacsha256_KEYBYTES (32 bytes)
    if (salt.size() > crypto_auth_hmacsha256_KEYBYTES) {
        // Hash long salts before use to preserve entropy
        uint8_t hashed_salt[crypto_auth_hmacsha256_KEYBYTES];
        crypto_hash_sha256(hashed_salt, salt.data(), salt.size());
        salt.assign(hashed_salt, hashed_salt + crypto_auth_hmacsha256_KEYBYTES);
    }
...
std::vector<OrchestratedPacket> ProtocolOrchestrator::send(
    ...
    // R7-ORCH-02: Check running_ to prevent race with stop()
    if (!running_.load() || !config_.enabled || payload.empty()) {
        OrchestratedPacket op;
        op.data = payload;
        return {op};
    }
...
void ProtocolOrchestrator::send_async(const std::vector<uint8_t>& payload) {
    // R7-ORCH-02: Check running_ to prevent race with stop()
    if (!running_.load() || !config_.enabled || payload.empty()) return;
...
ThreatLevel ProtocolOrchestrator::get_threat_level() const {
    // R9-H04: Thread-safe read of threat_level_
    std::lock_guard<std::mutex> lock(strategy_mutex_);
    return threat_level_;
}

// R11-FIX-11: Thread-safe threat level management
void ProtocolOrchestrator::set_threat_level(ThreatLevel level) {
    std::unique_lock<std::mutex> lock(strategy_mutex_);
    
    // Read old value under lock to prevent race condition
...
void ProtocolOrchestrator::apply_strategy(const OrchestratorStrategy& strategy) {
    ...
    if (strategy.enable_flow_shaping) {
        flow_shaper_.update_config(strategy.flow_config);
    }
...
OrchestratorStrategy ProtocolOrchestrator::get_strategy() const {
    // R9-H04: Thread-safe read of current_strategy_
    std::lock_guard<std::mutex> lock(strategy_mutex_);
...
OrchestratorConfig ProtocolOrchestrator::get_config() const {
    // R9-H04: Thread-safe read of config_
    std::lock_guard<std::mutex> lock(strategy_mutex_);
    return config_;
}

OrchestratorStats ProtocolOrchestrator::get_stats() const {
    // R9-H04: Thread-safe snapshot of stats_
    std::lock_guard<std::mutex> lock(strategy_mutex_);
