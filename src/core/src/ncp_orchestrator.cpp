...
void ProtocolOrchestrator::derive_dummy_key_from_secret_(
    ...
    // Derive a deterministic 32-byte dummy key from the shared secret
    // using HKDF so both peers get the same key without extra exchange.
    //
    // HKDF-SHA256(ikm=secret, salt="NCP-ADV-DUMMY-KEY-v1", info="", len=32)
    static const std::string salt_str = "NCP-ADV-DUMMY-KEY-v1";
    std::vector<uint8_t> salt(salt_str.begin(), salt_str.end());
    
    // Use libsodium's HKDF (crypto_kdf_hkdf_sha256)
    ...
    // info = "NCP-ADV-DUMMY" (context), output = 32 bytes (single block)
    static const std::string info_str = "NCP-ADV-DUMMY";
    uint8_t expand_input[64];  // info + 0x01
    std::memcpy(expand_input, info_str.data(), info_str.size());
    expand_input[info_str.size()] = 0x01;
    
    uint8_t okm[crypto_auth_hmacsha256_BYTES];  // 32 bytes = exactly what we need
...
std::vector<uint8_t> ProtocolOrchestrator::receive(
    ...
    // FIX: Use instance method with HMAC-derived session marker
    // (also checks legacy 0xDEADBEEF for backward compatibility)
    if (snapshot.enable_adversarial) {
