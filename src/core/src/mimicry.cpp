...
// R11-FIX-03: TLS record size limits per RFC 5246/8446
constexpr size_t MAX_TLS_RECORD_SIZE = 16384;      // 2^14 bytes (RFC 5246)
constexpr size_t MAX_TLS_RECORD_OVERHEAD = 5;       // ContentType(1) + Version(2) + Length(2)
constexpr size_t MAX_TLS_HANDSHAKE_SIZE = 65536;   // 2^16 bytes (RFC 5246)

// ==================== Russian whitelist domains & paths ====================
// R10-H03: This whitelist is hardcoded for regional traffic mimicry.
// To customize: modify the arrays below or add a config file parser.
// Environment variable override: NCP_RU_WHITELIST_FILE=/path/to/whitelist.json
//
// TODO(R13): Move to external config file (e.g., config/regional_whitelist.json)
// to allow user customization without recompilation.
...
// R10-H03: Configurable paths for Russian domain mimicry
...
// R10-H03: User agents for Russian traffic simulation
...
// ==================== Safe hex decode helper ====================
// R13-H03: Use simple nibble decoding instead of incorrect sodium_hex_decode usage.
// sodium_hex_decode expects (output, hex_string, hex_string_length), not bytes-to-decode.
// For DNS label encoding, we decode hex pairs manually with constant-time comparison.
static int hex_char_to_nibble(uint8_t c) {
    // Constant-time hex character to nibble conversion
    // Returns -1 for invalid characters
    unsigned int result = c - '0';
    if (result <= 9) return static_cast<int>(result);
    
    result = (c | 0x20) - 'a';  // Case-insensitive: 'A' -> 'a'
    if (result <= 5) return static_cast<int>(result + 10);
    
    return -1;  // Invalid hex character
}

// R10-H07: Decode a single byte from two hex characters
static bool safe_hex_decode_byte(uint8_t hi, uint8_t lo, uint8_t& out) {
    int h = hex_char_to_nibble(hi);
    int l = hex_char_to_nibble(lo);
    if (h < 0 || l < 0) return false;  // Invalid hex character
    out = static_cast<uint8_t>((h << 4) | l);
    return true;
}

// R11-H03: Decode hex string to unsigned integer
static unsigned int safe_hex_to_uint(const char* hex, size_t len) {
    unsigned int result = 0;
    for (size_t i = 0; i < len; ++i) {
        int n = hex_char_to_nibble(static_cast<uint8_t>(hex[i]));
        if (n < 0) return 0;  // Invalid hex character
...
TrafficMimicry::~TrafficMimicry() noexcept {
...
std::vector<uint8_t> TrafficMimicry::wrap_payload(
    ...
    if (config_.tls_key_rotation_packets > 0) {
        if (tls_packets_since_key_rotation_.fetch_add(1, std::memory_order_acq_rel) >= config_.tls_key_rotation_packets) {
    ...
    switch (profile) {
        case TrafficMimicry::MimicProfile::HTTP_GET:           result = create_http_get_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::HTTP_POST:          result = create_http_post_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::HTTPS_CLIENT_HELLO: result = create_https_client_hello_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::HTTPS_APPLICATION:  result = create_https_application_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::DNS_QUERY:          result = create_dns_query_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::DNS_RESPONSE:       result = create_dns_response_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::QUIC_INITIAL:       result = create_quic_initial_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::WEBSOCKET:          result = create_websocket_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::BITTORRENT:         result = create_bittorrent_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::SKYPE:              result = create_skype_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::ZOOM:               result = create_zoom_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::GENERIC_TCP:        result = create_generic_tcp_wrapper(payload); break;
        case TrafficMimicry::MimicProfile::GENERIC_UDP:
...
std::vector<uint8_t> TrafficMimicry::unwrap_payload(
    const std::vector<uint8_t>& mimicked_data, TrafficMimicry::MimicProfile profile) {
    std::vector<uint8_t> result;
    switch (profile) {
        case TrafficMimicry::MimicProfile::HTTP_GET:
        case TrafficMimicry::MimicProfile::HTTP_POST:           result = extract_http_payload(mimicked_data); break;
        case TrafficMimicry::MimicProfile::HTTPS_CLIENT_HELLO:
        case TrafficMimicry::MimicProfile::HTTPS_APPLICATION:   result = extract_tls_payload(mimicked_data); break;
        case TrafficMimicry::MimicProfile::DNS_QUERY:
        case TrafficMimicry::MimicProfile::DNS_RESPONSE:        result = extract_dns_payload(mimicked_data); break;
        case TrafficMimicry::MimicProfile::QUIC_INITIAL:        result = extract_quic_payload(mimicked_data); break;
        case TrafficMimicry::MimicProfile::WEBSOCKET:           result = extract_websocket_payload(mimicked_data); break;
        case TrafficMimicry::MimicProfile::BITTORRENT:          result = extract_bittorrent_payload(mimicked_data); break;
        default:
...
TrafficMimicry::MimicProfile TrafficMimicry::detect_profile(const std::vector<uint8_t>& data) {
    if (data.size() < 2) return TrafficMimicry::MimicProfile::GENERIC_UDP;

    if (data[0] == 0x16 || data[0] == 0x17) {
        if (data.size() >= 5 && data[1] == 0x03) {
            return (data[0] == 0x16) ? TrafficMimicry::MimicProfile::HTTPS_CLIENT_HELLO : TrafficMimicry::MimicProfile::HTTPS_APPLICATION;
        }
    }

    if (data.size() >= 4) {
        if (data[0]=='G' && data[1]=='E' && data[2]=='T' && data[3]==' ') return TrafficMimicry::MimicProfile::HTTP_GET;
    }
    if (data.size() >= 5) {
        if (data[0]=='P' && data[1]=='O' && data[2]=='S' && data[3]==' ' && data[4]==' ') return TrafficMimicry::MimicProfile::HTTP_POST;
    }

    if (data.size() >= 20 && data[0] == 19) {
        static const char* bt_proto = "BitTorrent protocol";
        if (std::memcmp(&data[1], bt_proto, 19) == 0) {
            return TrafficMimicry::MimicProfile::BITTORRENT;
        }
    }

    if (data.size() >= 5 && (data[0] & 0x80)) {
        if (data[1] == 0x00 && data[2] == 0x00 && data[3] == 0x00 && data[4] == 0x01) {
            return TrafficMimicry::MimicProfile::QUIC_INITIAL;
        }
    }
    ...
    if (data.size() >= 12) {
        ...
        if (opcode == 0 && qdcount >= 1 && qdcount <= 16) {
            if (qr == 0) return TrafficMimicry::MimicProfile::DNS_QUERY;
            if (qr == 1) return TrafficMimicry::MimicProfile::DNS_RESPONSE;
        }
    }
    ...
    if (data.size() >= 6) {
        ...
        if (fin && masked && opcode >= 0x01 && opcode <= 0x0A) {
            ...
            if (data.size() >= header_len) {
                return TrafficMimicry::MimicProfile::WEBSOCKET;
            }
        }
    }

    return TrafficMimicry::MimicProfile::GENERIC_TCP;
}
...
std::chrono::milliseconds TrafficMimicry::get_next_packet_delay() {
    ...
    return calculate_realistic_delay(cfg_snap, cfg_snap.profile, 0);
}
...
std::chrono::milliseconds TrafficMimicry::calculate_realistic_delay(
    ...
    switch (profile) {
        case TrafficMimicry::MimicProfile::HTTP_GET:
        case TrafficMimicry::MimicProfile::HTTP_POST:           base_delay = 75;  jitter = 50; break;
        case TrafficMimicry::MimicProfile::HTTPS_CLIENT_HELLO:
        case TrafficMimicry::MimicProfile::HTTPS_APPLICATION:   base_delay = 100; jitter = 75; break;
        case TrafficMimicry::MimicProfile::DNS_QUERY:
        case TrafficMimicry::MimicProfile::DNS_RESPONSE:        base_delay = 20;  jitter = 15; break;
        case TrafficMimicry::MimicProfile::QUIC_INITIAL:        base_delay = 50;  jitter = 30; break;
        case TrafficMimicry::MimicProfile::WEBSOCKET:           base_delay = 10;  jitter = 10; break;
        case TrafficMimicry::MimicProfile::SKYPE:
        case TrafficMimicry::MimicProfile::ZOOM:                base_delay = 20;  jitter = 5;  break;
        case TrafficMimicry::MimicProfile::BITTORRENT:          base_delay = 200; jitter = 150; break;
        default:                                                base_delay = 50;  jitter = 25; break;
    }

    base_delay += static_cast<int>(packet_size / 1000);
    int final_delay = std::max(cfg.min_inter_packet_delay,
                      std::min(cfg.max_inter_packet_delay,
...
// FIX: Validate hostname to prevent HTTP header injection
static bool is_valid_hostname(const std::string& host) {
    if (host.empty() || host.size() > 253) return false;
    
    // Allow only: a-z, A-Z, 0-9, hyphen, dot
    for (char c : host) {
...
static std::string sanitize_hostname(const std::string& host) {
...
std::vector<uint8_t> TrafficMimicry::create_http_get_wrapper(const std::vector<uint8_t>& payload) {
    ...
    std::string host = sanitize_hostname(generate_random_hostname());
...
std::vector<uint8_t> TrafficMimicry::create_https_client_hello_wrapper(const std::vector<uint8_t>& payload) {
    ...
    // FIX: If ciphertext is too large, split into multiple TLS records
    if (ciphertext.size() > MAX_TLS_RECORD_SIZE - 128) {
        // Split payload and create multiple records
        return create_fragmented_tls_records(payload, sni);
    }
    ...
    for (int i = 0; i < 7; ++i) result.push_back(ncp::csprng_bytes(1)[0]);
    ...
    if (remaining_ct > 0) {
        ...
        for (int i = 0; i < 4; ++i) exts.push_back(ncp::csprng_bytes(1)[0]);
        ...
        for (uint16_t i = 0; i < binder_len; ++i) exts.push_back(ncp::csprng_bytes(1)[0]);
    }
    ...
    // R12-FIX-01: Graceful fallback instead of exception in hot path
    if (total_len > MAX_TLS_RECORD_SIZE) {
        NCP_LOG_ERROR("[TrafficMimicry] TLS record size " + std::to_string(total_len) + 
                      " exceeds maximum " + std::to_string(MAX_TLS_RECORD_SIZE) + 
                      ", using fragmentation");
        // Return fragmented version instead of throwing
        return create_fragmented_tls_records(payload, sni);
    }
...
// FIX: Create fragmented TLS records for large payloads
std::vector<uint8_t> TrafficMimicry::create_fragmented_tls_records(
    const std::vector<uint8_t>& payload, const std::string& sni) {
    
    // Split payload into chunks that fit in MAX_TLS_RECORD_SIZE
    constexpr size_t CHUNK_SIZE = MAX_TLS_RECORD_SIZE - 256;  // Reserve space for headers
    ...
    while (offset < payload.size()) {
        size_t chunk_size = std::min(CHUNK_SIZE, payload.size() - offset);
        std::vector<uint8_t> chunk(payload.begin() + offset, payload.begin() + offset + chunk_size);
        
        // Create TLS record header for each fragment
        uint8_t nonce[crypto_aead_xchacha20poly1305_ietf_NPUBBYTES];
        randombytes_buf(nonce, sizeof(nonce));
        
        std::vector<uint8_t> ciphertext(chunk.size() + crypto_aead_xchacha20poly1305_ietf_ABYTES);
        unsigned long long ct_len = 0;
        ...
        result.push_back(0x03);  // TLS version
        result.push_back(0x01);
        // R6-04: Record body length must include 4-byte fragment header + 24-byte nonce + ciphertext
        size_t record_body_len = 4 + sizeof(nonce) + ciphertext.size();
        result.push_back(static_cast<uint8_t>(record_body_len >> 8));
        result.push_back(static_cast<uint8_t>(record_body_len & 0xFF));

        // Fragment header
        result.push_back(0x01);  // ClientHello
        result.push_back(fragment_id);
        result.push_back(static_cast<uint8_t>((ciphertext.size()) >> 8));
        result.push_back(static_cast<uint8_t>(ciphertext.size() & 0xFF));
...
std::vector<uint8_t> TrafficMimicry::create_quic_initial_wrapper(const std::vector<uint8_t>& payload) {
    std::vector<uint8_t> result;
    result.push_back(0xC0 | (ncp::csprng_bytes(1)[0] & 0x03));
    ...
    result.push_back(8); for (int i = 0; i < 8; ++i) result.push_back(ncp::csprng_bytes(1)[0]); // SCID
...
std::vector<uint8_t> TrafficMimicry::create_bittorrent_wrapper(const std::vector<uint8_t>& payload) {
    ...
    for (int i = 0; i < 20; ++i) res.push_back(ncp::csprng_bytes(1)[0]);
    const char* prefix = "-qB4630-";
    res.insert(res.end(), prefix, prefix + 8);
    for (int i = 0; i < 12; ++i) res.push_back(ncp::csprng_bytes(1)[0]);
    ...
    for (int i = 0; i < 4; ++i) res.push_back(ncp::csprng_bytes(1)[0]); // index
...
std::vector<uint8_t> TrafficMimicry::create_skype_wrapper(const std::vector<uint8_t>& payload) {
    std::vector<uint8_t> res;
    res.push_back(ncp::csprng_bytes(1)[0]); res.push_back(ncp::csprng_bytes(1)[0]);
    ...
    res.insert(res.end(), payload.begin(), payload.end());
    while (res.size() < 160) res.push_back(ncp::csprng_bytes(1)[0]);
    return res;
}
...
std::vector<uint8_t> TrafficMimicry::create_zoom_wrapper(const std::vector<uint8_t>& payload) {
    ...
    for (int i = 0; i < 4; ++i) res.push_back(ncp::csprng_bytes(1)[0]);
...
std::vector<uint8_t> TrafficMimicry::create_fake_server_hello() {
    ...
    for (int i=0;i<32;++i) res.push_back(ncp::csprng_bytes(1)[0]);
    res.push_back(32); for (int i=0;i<32;++i) res.push_back(ncp::csprng_bytes(1)[0]);
...
std::vector<uint8_t> TrafficMimicry::create_fake_finished() {
    ...
    for (int i=0; i<40; ++i) res.push_back(ncp::csprng_bytes(1)[0]);
    return res;
}
