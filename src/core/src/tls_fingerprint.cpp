...
void TLSFingerprint::encrypt_sni(const std::vector<uint8_t>& public_key) {
    ...
    // HPKE suite header
    // R7-03: Use correct AEAD ID for XChaCha20-Poly1305
    // RFC 9180: 0x0003 = ChaCha20-Poly1305 (12-byte nonce)
    // We use XChaCha20-Poly1305 (24-byte nonce), so use custom ID 0xFF03
    ...
    pImpl->encrypted_sni_data.push_back(0xFF); pImpl->encrypted_sni_data.push_back(0x03); // XChaCha20-Poly1305 (custom ID)
