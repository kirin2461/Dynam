...
#include <ws2tcpip.h>
#include <windows.h>
#include <fwpmu.h>     // FwpmEngineOpen0, FwpmSubLayerAdd0, FwpmFilterAdd0, etc.
#ifdef HAVE_WDK
#include <fwpsk.h>     // FwpsInjectionHandleCreate0 — requires WDK
#endif
#include <rpcdce.h>    // UuidCreate
#include <sstream>     // std::ostringstream (for wfp_err)
...
public:
    ...
    bool initialize(const Config& cfg) override {
        ...
        if (!nfq_handle_) {
            parent->log_from_backend("[NFQUEUE] nfq_open() failed");
            return false;
        }
        ...
        if (nfq_bind_pf(nfq_handle_, AF_INET) < 0) {
            parent->log_from_backend("[NFQUEUE] nfq_bind_pf() failed");
        ...
        nfq_queue_ = nfq_create_queue(nfq_handle_, cfg.nfqueue_num,
                                      ...
                                      this);
        if (!nfq_queue_) {
            parent->log_from_backend("[NFQUEUE] nfq_create_queue() failed (queue=" +
        ...
        if (fd_ < 0) {
            parent->log_from_backend("[NFQUEUE] nfq_fd() failed");
        ...
        if (!iptables_exec("-A", cfg.nfqueue_num)) {
            parent->log_from_backend("[NFQUEUE] Warning: Failed to add iptables rule. "
        ...
        parent->log_from_backend("[NFQUEUE] Initialized on queue " + std::to_string(cfg.nfqueue_num));
        return true;
    }
    ...
    bool start() override {
        ...
        worker_thread_ = std::thread([this]() {
            char buf[4096];
            int rv;
            parent->log_from_backend("[NFQUEUE] Worker thread started");
            while (running_) {
            ...
            parent->log_from_backend("[NFQUEUE] Worker thread stopped");
        });
        return true;
    }
...
private:
                        ...
                        struct nfq_data* nfa)
    {
        ...
        parent->backend_stats().packets_intercepted++;
        parent->backend_stats().bytes_processed += packet.size();

        // Call packet handler
        PacketInterceptor::Verdict verdict;
        verdict = parent->invoke_packet_handler(packet, true);
        ...
        switch (verdict) {
            ...
            case PacketInterceptor::Verdict::DROP:
                nf_verdict = NF_DROP;
                parent->backend_stats().packets_dropped++;
                break;
            case PacketInterceptor::Verdict::MODIFIED:
                nf_verdict = NF_ACCEPT;
                parent->backend_stats().packets_modified++;
...
public:
    ...
    bool initialize(const Config& cfg) override {
        ...
        if (handle_ == INVALID_HANDLE_VALUE) {
            ...
            parent->log_from_backend("[WinDivert] WinDivertOpen failed, error=" + std::to_string(err));
            if (err == ERROR_FILE_NOT_FOUND) {
                parent->log_from_backend("[WinDivert] WinDivert driver not found. "
                           "Ensure WinDivert.sys and WinDivert.dll are in PATH or application directory.");
            } else if (err == ERROR_ACCESS_DENIED) {
                parent->log_from_backend("[WinDivert] Access denied. Run as Administrator.");
            }
            return false;
        }
        ...
        parent->log_from_backend("[WinDivert] Initialized with filter: " + cfg.windivert_filter);
        return true;
    }
    ...
    bool start() override {
        ...
        worker_thread_ = std::thread([this]() {
            ...
            parent->log_from_backend("[WinDivert] Worker thread started");
            ...
            while (running_) {
                                   ...
                                   &recv_len, &addr))
                {
                    ...
                    parent->log_from_backend("[WinDivert] WinDivertRecv failed, error=" + std::to_string(err));
                    break;
                }
                ...
                PacketInterceptor::Verdict verdict;
                verdict = parent->invoke_packet_handler(packet, is_outbound);
            ...
            parent->log_from_backend("[WinDivert] Worker thread stopped");
        });

        parent->log_from_backend("[WinDivert] Started");
        return true;
    }
...
//
// Architecture:
//   initialize()  — open engine, begin transaction, add sublayer, add permit/block filters
//   start()       — mark running; WFP filters are active as soon as they are added
//   stop()        — remove filters, remove sublayer, close engine
//   inject_packet() — inject a raw IP packet via FwpsInjectNetworkSendAsync0
//
// NOTE: WFP callout drivers (FwpmCalloutAdd0) require a signed kernel-mode driver.
// Userspace (UMDF) callouts are NOT supported by fwpuclnt.lib on standard Windows.
// Therefore this backend uses classify filters (permit/block) rather than callouts.
// For full packet inspection and modification, use WinDivertBackend (HAVE_WINDIVERT).
//
// All WFP API calls are guarded by #ifdef _WIN32 and use proper GUID generation via
// UuidCreate() so each session gets unique filter/sublayer identifiers.

// Helper: generate a random GUID for sublayers and filters
static inline GUID wfp_generate_guid() {
    GUID g;
    UuidCreate(&g);
    return g;
}

// Helper: convert DWORD WFP error to hex string for logging
static inline std::string wfp_err(DWORD d) {
    std::ostringstream oss;
    oss << "0x" << std::hex << std::uppercase << d;
    return oss.str();
}
...
public:
    ...
    bool initialize(const Config& cfg) override {
        ...
        // ── 1. Open WFP engine (dynamic session, auto-commit on close) ──
        FWPM_SESSION0 session{};
        session.flags = FWPM_SESSION_FLAG_DYNAMIC; // filters removed automatically on engine close
        DWORD r = FwpmEngineOpen0(
            nullptr,           // local BFE
            RPC_C_AUTHN_WINNT, // Windows authentication
            nullptr,           // explicit credentials (nullptr = current user)
            &session,
            &engine_handle_
        );
        if (r != ERROR_SUCCESS) {
            parent->log_from_backend("[WFP] FwpmEngineOpen0 failed: " + wfp_err(r) +
                        ". Run as Administrator and ensure BFE service is running.");
            engine_handle_ = nullptr;
            return false;
        }

        // ── 2. Begin transaction ──
        r = FwpmTransactionBegin0(engine_handle_, 0);
        if (r != ERROR_SUCCESS) {
            parent->log_from_backend("[WFP] FwpmTransactionBegin0 failed: " + wfp_err(r));
        ...
        // ── 3. Add sublayer ──
        sublayer_guid_ = wfp_generate_guid();

        FWPM_SUBLAYER0 sublayer{};
        sublayer.subLayerKey = sublayer_guid_;
        // Convert std::string name to wide string for the FWPM_DISPLAY_DATA0
        std::wstring wname(cfg.wfp_sublayer_name.begin(), cfg.wfp_sublayer_name.end());
        sublayer.displayData.name        = const_cast<wchar_t*>(wname.c_str());
        sublayer.displayData.description = const_cast<wchar_t*>(L"NCP packet interceptor sublayer");
        sublayer.flags  = 0;
        sublayer.weight = static_cast<UINT16>(cfg.wfp_weight & 0xFFFF);

        r = FwpmSubLayerAdd0(engine_handle_, &sublayer, nullptr);
        if (r != ERROR_SUCCESS) {
            parent->log_from_backend("[WFP] FwpmSubLayerAdd0 failed: " + wfp_err(r));
        ...
        // ── 4. Add a permit filter at the IPv4 outbound transport layer ──
        // This is the userspace equivalent of registering a callout:
        // We add a filter that permits all outbound IPv4 traffic through our sublayer.
        // A real callout driver would intercept packets here; in userspace we use this
        // sublayer as an anchor and rely on WinDivert for actual packet modification.
        filter_guid_ = wfp_generate_guid();

        FWPM_FILTER0 filter{};
        filter.filterKey       = filter_guid_;
        std::wstring wfname   = wname + L"_OutboundPermit";
        filter.displayData.name        = const_cast<wchar_t*>(wfname.c_str());
        filter.displayData.description = const_cast<wchar_t*>(L"NCP outbound permit filter");
        filter.flags           = 0;
        filter.layerKey        = FWPM_LAYER_OUTBOUND_TRANSPORT_V4; // IPv4 outbound transport
        filter.subLayerKey     = sublayer_guid_;
        filter.weight.type     = FWP_UINT64;
        static UINT64 weight_val = cfg.wfp_weight;
        filter.weight.uint64   = &weight_val;
        filter.numFilterConditions = 0; // no conditions = matches all traffic
        filter.filterCondition = nullptr;
        filter.action.type     = FWP_ACTION_PERMIT; // permit outbound
        UINT64 filter_id       = 0;

        r = FwpmFilterAdd0(engine_handle_, &filter, nullptr, &filter_id);
        if (r != ERROR_SUCCESS) {
            parent->log_from_backend("[WFP] FwpmFilterAdd0 failed: " + wfp_err(r));
            // Non-fatal: sublayer is still added, we can still track the session
            parent->log_from_backend("[WFP] Warning: outbound permit filter not installed. "
                        "WFP session is active but traffic interception is inactive.");
        } else {
            filter_id_ = filter_id;
            filter_added_ = true;
            parent->log_from_backend("[WFP] Outbound permit filter installed (filter_id=" +
                        std::to_string(filter_id) + ")");
        }

        // ── 5. Commit transaction ──
        r = FwpmTransactionCommit0(engine_handle_);
        if (r != ERROR_SUCCESS) {
            parent->log_from_backend("[WFP] FwpmTransactionCommit0 failed: " + wfp_err(r));
            FwpmTransactionAbort0(engine_handle_);
            FwpmEngineClose0(engine_handle_);
            engine_handle_ = nullptr;
            sublayer_added_ = false;
            filter_added_   = false;
            return false;
        }

        parent->log_from_backend("[WFP] Initialized: engine open, sublayer '" + cfg.wfp_sublayer_name +
                    "' added. Note: full packet interception requires WINDIVERT backend.");
        return true;
    }
    ...
    bool start() override {
        if (running_) return true;
        running_ = true;
        parent->log_from_backend("[WFP] Started. WFP sublayer is active. "
                    "For packet modification use Backend::WINDIVERT or Backend::AUTO.");
        return true;
    }
    ...
    void stop() override {
        ...
        // With FWPM_SESSION_FLAG_DYNAMIC these are also cleaned up on engine close,
        // but explicit removal is cleaner and avoids BFE audit log noise.
        if (FwpmTransactionBegin0(engine_handle_, 0) == ERROR_SUCCESS) {
            if (filter_added_) {
                FwpmFilterDeleteByKey0(engine_handle_, &filter_guid_);
                filter_added_ = false;
            }
            if (sublayer_added_) {
                FwpmSubLayerDeleteByKey0(engine_handle_, &sublayer_guid_);
                sublayer_added_ = false;
            }
            FwpmTransactionCommit0(engine_handle_);
        }

        FwpmEngineClose0(engine_handle_);
        engine_handle_ = nullptr;

        parent->log_from_backend("[WFP] Stopped. Filters and sublayer removed.");
    }

    bool is_running() const override { return running_.load(); }

    // ── inject_packet: send a raw IPv4 packet via WFP injection handle ──
    // Uses FwpsInjectNetworkSendAsync0 to inject an outbound packet at the
    // network layer. Requires SeNetworkPrivilege (Administrator).
    //
    // Parameters:
    //   packet_data  — raw IPv4 packet bytes (with IPv4 header)
    //   packet_len   — length in bytes
    //
    // Returns true if the inject call was successfully queued.
    bool inject_packet(const uint8_t* packet_data, size_t packet_len) {
        if (!engine_handle_ || !packet_data || packet_len == 0) return false;

        // Open an injection handle once (lazy init, protected by mutex)
        {
...
#else
                // WDK not available — injection not supported
                return false;
#endif
            }
        }
        ...
        if (dest) {
            std::memcpy(dest, packet_data, packet_len);
        } else {
        ...
        r = FwpsInjectNetworkSendAsync0(
            ...
            nbl,
            [](void* /*ctx*/, NET_BUFFER_LIST* list, BOOLEAN /*dispatch*/) {
                // Completion callback: free the NBL
                FwpsFreeNetBufferList0(list);
            },
        ...
        return true; // NBL will be freed in completion callback
#else
        // WDK not available — injection not supported
        (void)packet_data; (void)packet_len;
        return false;
#endif
    }
