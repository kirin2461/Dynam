...
// R14-H05: Maximum secure buffer allocation limit (100MB)
// Prevents DoS via excessive memory allocation requests
static constexpr size_t MAX_SECURE_BUFFER_SIZE = 1024 * 1024 * 100;
...
SecureBuffer::SecureBuffer(size_t size)
    ...
    // R14-H05: Validate size to prevent DoS via excessive allocation
    if (size > MAX_SECURE_BUFFER_SIZE) {
        throw std::runtime_error("SecureBuffer size exceeds maximum (100MB)");
    }
...
SecureBuffer::SecureBuffer(const std::vector<uint8_t>& data)
    ...
    // R14-H05: Validate size to prevent DoS via excessive allocation
    if (size_ > MAX_SECURE_BUFFER_SIZE) {
        throw std::runtime_error("SecureBuffer size exceeds maximum (100MB)");
    }
