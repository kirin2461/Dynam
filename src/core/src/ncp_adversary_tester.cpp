...
ReferenceProfile ReferenceProfile::gaming() {
    // Online game traffic profile (UDP-based, e.g. FPS / real-time strategy).
    //
    // Characteristics:
    //   Packet sizes:  predominantly 40–150 bytes (player state, inputs, heartbeats)
    //                  with periodic 500–1500-byte bursts (world-state snapshots,
    //                  asset delta sync, server-side broadcast).
    //   Timing:        ~16 ms base interval (60 fps tick rate)
    //                  with ±5 ms jitter (network conditions, OS scheduler).
    //                  Bursts of 2–4 packets within 1–3 ms (state reconciliation).
    //   Symmetry:      roughly symmetric upload/download (bidirectional game state).
    //   Variance:      moderate — much lower than web (no large HTTP objects),
    //                  higher than messenger (periodic large state updates).
    //   Burst ratio:   moderate — short bursts at state sync intervals (≈3x avg rate),
    //                  without the long quiet gaps of video streaming.
    //
    // CDF breakpoints reflect ~80% of packets in 40–150 byte range,
    // ~15% in 150–500 range (control packets, audio), ~5% up to 1460 (state dumps).
    ReferenceProfile p;
    p.name = "gaming";
    p.size_buckets = {64, 128, 256, 512, 1024, 1460};
    //                small game pkts   ctrl  audio  state  MTU
    p.size_cdf     = {0.45, 0.78, 0.89, 0.95, 0.98, 1.0};
    // Variance: mix of tiny (40 B) and periodic large (1000 B) gives ~22000
    // (empirically measured on CS2 / Valorant traces)
    p.expected_size_variance = 22000.0;
    // Timing CoV: 60fps ±5ms jitter → mean=16ms, stddev≈5ms → CoV≈0.31
    p.expected_timing_cov = 0.31;
    // Burst ratio: short bursts of 3 packets at 16ms intervals on state sync
    p.expected_burst_ratio = 3.0;
    return p;
}
    ...
    const ReferenceProfile& reference)
{
    ...
    // TODO(R13): For large windows (>1000 packets), pre-sort observed_sizes
