...
static UniqueEVP_PKEY ec_p256_pkey_from_private_legacy(const uint8_t* priv_raw, size_t priv_len,
                                       const uint8_t* pub_raw, size_t pub_len)
{
    ...
    if (pub_raw && pub_len > 0) {
        UniqueBN_CTX ctx(BN_CTX_new());
        UniqueEC_POINT pub_point(EC_POINT_new(EC_KEY_get0_group(ec_key.get())));
        if (EC_POINT_oct2point(EC_KEY_get0_group(ec_key.get()), pub_point.get(), pub_raw, pub_len, ctx.get()) != 1) {
    ...
    if (EVP_PKEY_set1_EC_KEY(pkey, ec_key.get()) != 1) {
        EVP_PKEY_free(pkey);
...
// R15-H02: Full RAII — no manual cleanup needed
static UniqueEVP_PKEY ec_p256_pkey_from_public_legacy(const uint8_t* pub_raw, size_t pub_len) {
    UniqueEC_KEY ec_key(EC_KEY_new_by_curve_name(NID_X9_62_prime256v1));
    if (!ec_key) throw std::runtime_error("EC_KEY_new_by_curve_name failed");
    ...
    if (EC_POINT_oct2point(EC_KEY_get0_group(ec_key.get()), pub_point.get(), pub_raw, pub_len, ctx.get()) != 1) {
        throw std::runtime_error("EC_POINT_oct2point failed for EC P-256 public key");
    }
                                       ...
                                       const uint8_t* pub_raw, size_t pub_len)
{
    ...
    // OpenSSL 3.0+ — use EVP_PKEY_fromdata
    ...
    return UniqueEVP_PKEY(pkey);
#else
    // OpenSSL 1.1.1 — use legacy EC_KEY API
    return ec_p256_pkey_from_private_legacy(priv_raw, priv_len, pub_raw, pub_len);
#endif
}
...
#else
    // OpenSSL 1.1.1 — use legacy EC_KEY API
    return ec_p256_pkey_from_public_legacy(pub_raw, pub_len);
#endif
}
...
struct E2ESession::Impl {
    ...
    void dh_ratchet_step(const std::vector<uint8_t>& new_remote_pub) {
        ...
        ratchet.chain_key = SecureMemory(sending_chain_key.size());
        std::memcpy(ratchet.chain_key.data(), new_send_chain.data(), new_send_chain.size());
    ...
    bool skip_message_keys(SecureMemory& recv_chain_key, uint32_t& recv_counter, uint32_t until) {
        ...
        // R6-SEC-01: Evict oldest keys if we're at the cap
        while (ratchet.skipped_keys.size() >= ratchet.max_skipped_keys && 
               !ratchet.skipped_keys.empty()) {
            ratchet.skipped_keys.erase(ratchet.skipped_keys.begin());
        }
        
        while (recv_counter < until) {
            // R6-SEC-01: Check cap before adding new key
            if (ratchet.skipped_keys.size() >= ratchet.max_skipped_keys) {
...
SecureMemory E2ESession::compute_shared_secret(const KeyPair& local_keypair, const std::vector<uint8_t>& peer_public_key) {
    ...
    if (local_keypair.protocol == KeyExchangeProtocol::Kyber1024) {
        ...
        // Otherwise, we're the sender → encapsulate
        if (pImpl_->last_kem_ciphertext.size() > 0) {
            // Receiver side: decapsulate using our private key and received ciphertext
            auto shared = decapsulate(local_keypair, 
                std::vector<uint8_t>(pImpl_->last_kem_ciphertext.data(), 
                                     pImpl_->last_kem_ciphertext.data() + pImpl_->last_kem_ciphertext.size()));
            return shared;
        } else {
            // Sender side: encapsulate using receiver's public key
...
void E2ESession::init_ratchet_keys() {
    ...
    SecureMemory tmp_chain(impl.sending_chain_key.size());
    std::memcpy(tmp_chain.data(), impl.sending_chain_key.data(), impl.sending_chain_key.size());
    impl.ratchet.chain_key = std::move(tmp_chain);
...
std::vector<uint8_t> E2ESession::create_key_exchange_request(const KeyPair& local_keys) {
    ...
    // For Kyber1024: encapsulate using the peer's public key (which should be known out-of-band)
    // For now, we just send our public key; ciphertext will be sent in response
    ...
    // For Kyber1024: reserve space for ciphertext (will be added by receiver in response)
    if (local_keys.protocol == KeyExchangeProtocol::Kyber1024) {
        // Ciphertext length placeholder (will be filled by receiver)
        append_u16(request, 0); // Placeholder for ciphertext length
    }
...
std::vector<uint8_t> E2ESession::process_key_exchange_request(const std::vector<uint8_t>& request, const KeyPair& local_keys) {
    ...
    // For Kyber1024: sender's request may include ciphertext placeholder
    // Receiver (server) encapsulates and sends ciphertext in response
    if (local_keys.protocol == KeyExchangeProtocol::Kyber1024) {
        // Receiver side: encapsulate using sender's public key (peer_public)
        std::vector<uint8_t> ciphertext;
        pImpl_->shared_secret = encapsulate(peer_public, ciphertext);
        pImpl_->last_kem_ciphertext = SecureMemory(ciphertext.size());
        std::memcpy(pImpl_->last_kem_ciphertext.data(), ciphertext.data(), ciphertext.size());
        
        // Build response with ciphertext
        ...
        // Append ciphertext
        append_u16(response, static_cast<uint16_t>(ciphertext.size()));
    ...
    // For DH-based protocols (X25519, X448, P256)
...
bool E2ESession::complete_key_exchange(const std::vector<uint8_t>& response, const KeyPair& local_keys) {
    ...
    // R7-SEC-03: Verify transcript hash with correct offset for each protocol.
    // For DH protocols (X25519, X448, P256): hash is at response[4 + pk_len]
    // For Kyber1024: hash is at response[4 + pk_len + 2 + ct_len] (after ciphertext)
    if (response[0] == 0x20) {
    ...
    // For Kyber1024: response includes ciphertext from receiver
    if (local_keys.protocol == KeyExchangeProtocol::Kyber1024 && response.size() >= 4u + pk_len + 2) {
        size_t ct_offset = 4 + pk_len;
        uint16_t ct_len = read_u16(response.data() + ct_offset);
        if (response.size() >= ct_offset + 2 + ct_len) {
            std::vector<uint8_t> ciphertext(response.begin() + ct_offset + 2,
                                            response.begin() + ct_offset + 2 + ct_len);
            // Store ciphertext for decapsulation in compute_shared_secret
            pImpl_->last_kem_ciphertext = SecureMemory(ct_len);
            std::memcpy(pImpl_->last_kem_ciphertext.data(), ciphertext.data(), ct_len);
            // Now compute_shared_secret will use decapsulate
            pImpl_->shared_secret = compute_shared_secret(local_keys, peer_public);
            pImpl_->remote_ratchet_pub = peer_public;
            init_ratchet_keys();
            pImpl_->state = E2ESessionState::SessionEstablished;
...
EncryptedMessage E2ESession::encrypt_message(
    ...
    if (crypto_aead_xchacha20poly1305_ietf_encrypt(
        ...
        throw std::runtime_error("E2ESession::encrypt_message: encryption failed");
    }
...
std::vector<uint8_t> E2ESession::decrypt_message(
    ...
    if (crypto_aead_xchacha20poly1305_ietf_decrypt(
        ...
        throw std::runtime_error("E2ESession::decrypt_message: decryption failed (auth tag mismatch)");
    }
...
std::optional<std::vector<uint8_t>> E2ESession::decrypt_with_key_(
    ...
    // R7-SEC-02: Reconstruct AAD with key commitment tag for verification.
    // The commitment tag is appended to the end of AAD during encryption.
    ...
    // Key commitment tag: BLAKE2b-256(message_key) - must match what sender computed
    std::vector<uint8_t> key_commitment(crypto_generichash_BYTES);
    crypto_generichash(key_commitment.data(), key_commitment.size(),
                       message_key.data(), message_key.size(),
                       nullptr, 0);
    aad.insert(aad.end(), key_commitment.begin(), key_commitment.end());
    sodium_memzero(key_commitment.data(), key_commitment.size());
...
void E2ESession::ratchet_sending_chain() {
    ...
    {
        SecureMemory chain_copy(pImpl_->sending_chain_key.size());
        std::memcpy(chain_copy.data(), pImpl_->sending_chain_key.data(), pImpl_->sending_chain_key.size());
        pImpl_->ratchet.chain_key = std::move(chain_copy);
    }
...
EncryptedMessage E2ESession::encrypt(const std::vector<uint8_t>& plaintext, const std::vector<uint8_t>& associated_data) {
    ...
    // R7-SEC-01: Force DH ratchet step before chain length reaches 2^32 - 1
    // to prevent message_number wraparound and nonce reuse.
    // Threshold: 2^31 messages (half of max) to provide safety margin.
    constexpr uint32_t MAX_CHAIN_LENGTH = 1u << 31;
    if (pImpl_->ratchet.sending_chain_length >= MAX_CHAIN_LENGTH) {
        throw std::runtime_error(
            "Sending chain length exceeded threshold. "
            "Application must perform DH ratchet step before continuing. "
            "See AUDIT.md #R7-SEC-01 for details.");
    }
    ...
    // R7-SEC-02: Add key commitment to XChaCha20-Poly1305 to prevent
    // invisible salamander attack. Compute BLAKE2b-256 of message_key
    // and include it in AAD.
    std::vector<uint8_t> aad;
    aad.push_back(hdr.version);
    append_u32(aad, hdr.message_number);
    aad.insert(aad.end(), hdr.dh_public_key.begin(), hdr.dh_public_key.end());
    ...
    // Key commitment tag: BLAKE2b-256(message_key)
    // This binds the ciphertext to the specific key, preventing an attacker
    // from crafting a ciphertext that decrypts under multiple keys.
    std::vector<uint8_t> key_commitment(crypto_generichash_BYTES);
    crypto_generichash(key_commitment.data(), key_commitment.size(),
                       message_key.data(), message_key.size(),
                       nullptr, 0);
...
std::optional<std::vector<uint8_t>> E2ESession::decrypt(const EncryptedMessage& encrypted_message) {
    ...
    // R7-SEC-04: Reject messages with already-consumed message numbers
    // to prevent chain rollback attacks. Without this check, an attacker
    // can send a forged message with msg_num=0 to permanently kill the session.
    if (msg_num < pImpl_->ratchet.receiving_chain_length) {
        return std::nullopt;  // Already consumed this message number
    }
...
bool E2ESession::restore_session_state(const std::vector<uint8_t>& data) {
    ...
    {
        SecureMemory restore_chain(pImpl_->sending_chain_key.size());
        std::memcpy(restore_chain.data(), pImpl_->sending_chain_key.data(), pImpl_->sending_chain_key.size());
        pImpl_->ratchet.chain_key = std::move(restore_chain);
    }
...
SecureMemory derive_key(
    ...
    crypto_auth_hmacsha512256_state st;
    if (salt.empty()) {
        std::vector<uint8_t> zero_salt(crypto_auth_hmacsha512256_KEYBYTES, 0);
        crypto_auth_hmacsha512256_init(&st, zero_salt.data(), zero_salt.size());
    } else {
        crypto_auth_hmacsha512256_init(&st, salt.data(), salt.size());
    }
    crypto_auth_hmacsha512256_update(&st, input_key_material.data(), input_key_material.size());
    crypto_auth_hmacsha512256_final(&st, prk);
...
SecureMemory hkdf_expand(
    ...
    size_t hash_len = crypto_auth_hmacsha512256_BYTES;
    size_t n = (length + hash_len - 1) / hash_len;
    if (n > 255) throw std::runtime_error("HKDF-Expand: output too long");
    ...
    for (size_t i = 1; i <= n; ++i) {
        ...
        if (!t_prev.empty()) {
            crypto_auth_hmacsha512256_update(&st, t_prev.data(), t_prev.size());
        }
        ...
        uint8_t t_buf[crypto_auth_hmacsha512256_BYTES];
        crypto_auth_hmacsha512256_final(&st, t_buf);

        size_t to_copy = std::min(hash_len, length - offset);
        std::memcpy(output.data() + offset, t_buf, to_copy);
        offset += to_copy;

        t_prev.assign(t_buf, t_buf + hash_len);
        sodium_memzero(t_buf, sizeof(t_buf));
    }
...
std::vector<uint8_t> pad_message(
    ...
    // ISO 7816-4 / PKCS#7 hybrid: append 0x80, then zeros to fill block
    size_t padded_len = ((message.size() + 1 + block_size - 1) / block_size) * block_size;
    std::vector<uint8_t> padded(padded_len, 0);
    std::memcpy(padded.data(), message.data(), message.size());
    padded[message.size()] = 0x80;  // ISO 7816-4 padding marker
...
std::optional<std::vector<uint8_t>> unpad_message(
    ...
    for (size_t i = padded_message.size(); i > 0; --i) {
        if (padded_message[i - 1] == 0x80) {
            return std::vector<uint8_t>(padded_message.begin(), padded_message.begin() + (i - 1));
        }
        if (padded_message[i - 1] != 0x00) {
            // Invalid padding
            return std::nullopt;
        }
    }
...
std::vector<uint8_t> serialize_message(const EncryptedMessage& msg) {
    ...
    append_u32(out, msg.header.message_number);
    append_u32(out, msg.header.previous_chain_length);
    append_blob(out, msg.header.dh_public_key);
    append_blob(out, msg.header.nonce);
    append_blob(out, msg.header.associated_data);
    // Body
    append_blob(out, msg.ciphertext);
    append_blob(out, msg.auth_tag);
    append_blob(out, msg.nonce);
    return out;
}
...
std::optional<EncryptedMessage> deserialize_message(
    ...
    EncryptedMessage msg;
    if (!safe_read(1)) return std::nullopt;
    msg.header.version = data[pos++];
    if (!safe_read(8)) return std::nullopt;
    msg.header.message_number = read_u32(data.data() + pos); pos += 4;
    msg.header.previous_chain_length = read_u32(data.data() + pos); pos += 4;
    if (!read_vec_blob(msg.header.dh_public_key)) return std::nullopt;
    if (!read_vec_blob(msg.header.nonce)) return std::nullopt;
    if (!read_vec_blob(msg.header.associated_data)) return std::nullopt;
    if (!read_vec_blob(msg.ciphertext)) return std::nullopt;
    if (!read_vec_blob(msg.auth_tag)) return std::nullopt;
    if (!read_vec_blob(msg.nonce)) return std::nullopt;
    return msg;
}
