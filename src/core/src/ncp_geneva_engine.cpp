...
/// R11-FIX-10: Safe unaligned access for all architectures (ARM, x86, MIPS)
static uint16_t ip_checksum(const void* data, size_t len) {
    const uint8_t* buf = static_cast<const uint8_t*>(data);
    uint32_t sum = 0;
    
    // R11-FIX-10: Byte-by-byte processing for unaligned buffers
    // Modern compilers optimize this to efficient unaligned loads where supported
    while (len >= 2) {
    ...
    const GenevaStrategy& strategy)
{
    ...
    for (const auto& step : strategy.steps) {
        // R7-02: Stop if we've hit the packet limit
        if (packets.size() >= MAX_PACKETS) {
            stats_.packets_dropped += packets.size() - MAX_PACKETS;
            packets.resize(MAX_PACKETS);
            break;
        }
...
{
    // R7-02: Prevent exponential packet growth DoS
    if (packets.size() >= MAX_PACKETS) {
        stats_.packets_dropped += packets.size() - MAX_PACKETS;
        auto result = packets;
        result.resize(MAX_PACKETS);
        return result;
    }
