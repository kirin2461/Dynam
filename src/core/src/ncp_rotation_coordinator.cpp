 ...
 *   - Callbacks (rotation_cb, correlation_cb) are snapshot under lock and
 *     invoked AFTER unlock to prevent use-after-move race with set_*_callback
 *     (R7-ORCH-01). This also prevents deadlock if callback re-enters coordinator.
...
struct RotationCoordinator::Impl {
    ...
    // Records event under lock, then snapshots callback and calls it
    // after unlock to prevent deadlock if callback re-enters coordinator.
    void exec_rotation_locked(RotationLayer layer) {
        record_event_locked(layer);
        // Note: rotation_cb will be snapshot by caller and invoked after unlock
    }
    ...
    // Snapshots correlation_cb under lock, calls after unlock.
    std::array<uint32_t, ROTATION_LAYER_COUNT> check_correlation_locked(
        CorrelationCallback& cb_snap)
    {
        last_corr.clear();
        ...
        for (size_t i = 0; i < ROTATION_LAYER_COUNT; ++i) {
            for (size_t j = i + 1; j < ROTATION_LAYER_COUNT; ++j) {
                ...
                if (cr.is_correlated) {
                    ...
                    // Snapshot callback under lock for later invocation
                    cb_snap = correlation_cb;
                }
    ...
    // --- Scheduler loop ---
    // R7-ORCH-01: Snapshot callbacks under lock, invoke after unlock
    // to prevent use-after-move race with set_*_callback.
    void scheduler_loop() {
        while (running.load()) {
            ...
            // R7-ORCH-01: Snapshot correlation callback under lock
            CorrelationCallback corr_cb_snap;
            auto per_layer_delay = check_correlation_locked(corr_cb_snap);
            ...
            // Snapshot rotation callback under lock before firing
            RotationCallback rot_cb_snap = rotation_cb;

            // Fire all non-delayed due layers
            std::vector<RotationLayer> layers_to_fire;
            for (size_t i : due_layers) {
                auto layer = static_cast<RotationLayer>(i);
                exec_coordinated_locked(layer);
                schedule_next_locked(layer);
                layers_to_fire.push_back(layer);
            }

            // Unlock before invoking callbacks
            lock.unlock();

            // Invoke rotation callbacks after unlock
            if (rot_cb_snap) {
                for (auto layer : layers_to_fire) {
                    rot_cb_snap(layer);
                }
            }

            // Invoke correlation callback after unlock (if detected)
            if (corr_cb_snap) {
                // Re-acquire lock to safely read last_corr
                std::lock_guard<std::mutex> lk(mu);
                for (const auto& cr : last_corr) {
                    if (cr.is_correlated) {
                        corr_cb_snap(cr);
                    }
                }
...
// R7-ORCH-01: Snapshot callback under lock, invoke after unlock
void RotationCoordinator::rotate_now(RotationLayer layer) {
    if (!impl_->running.load()) return;

    RotationCallback cb_snap;
    {
        std::lock_guard<std::mutex> lk(impl_->mu);
        impl_->exec_coordinated_locked(layer);
        impl_->schedule_next_locked(layer);
        cb_snap = impl_->rotation_cb;  // snapshot under lock
    }
    impl_->cv.notify_all();

    // Invoke callback after unlock
    if (cb_snap) cb_snap(layer);
}
