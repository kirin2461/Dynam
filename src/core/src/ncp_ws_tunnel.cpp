    ...
    enum lws_callback_reasons reason, void* user, void* in, size_t len)
{
    ...
    case LWS_CALLBACK_CANCELLED:
        // Wakeup from lws_cancel_service — request writable if data pending
        {
            std::lock_guard<std::mutex> lock(self->send_mutex);
            if (!self->send_queue.empty() && self->connected.load())
                lws_callback_on_writable(wsi);
        }
        break;
#endif
...
bool WSTunnel::send(const uint8_t* data, size_t len)
{
    ...
    // R6-09: Use lws_cancel_service to wake up the service thread instead of
    // calling lws_callback_on_writable directly from a non-service thread.
    // lws_callback_on_writable must only be called from within lws callback context.
    if (impl_->wsi && impl_->context) {
        lws_cancel_service(impl_->context);
    }
