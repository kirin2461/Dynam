...
// ===== CSPRNG =====
// FIX #68: Replace raw /dev/urandom read with randombytes_buf() from libsodium.
...
PortKnock::PortKnock() : PortKnock(PortKnockConfig::balanced()) {}
PortKnock::PortKnock(const PortKnockConfig& config) : config_(config) {}
    ...
    const uint8_t* data, size_t data_len) const 
{
    ...
    if (secret.size() == crypto_auth_KEYBYTES) {
        ...
        crypto_generichash(key, crypto_auth_KEYBYTES, secret.data(), secret.size(), nullptr, 0);
    }
    lock.unlock();
...
KnockResult PortKnock::process_knock(
    const std::string& source_ip, uint16_t knocked_port) 
{
    ...
    if (mode == KnockMode::TOTP_SEQUENCE) {
        for (int64_t offset = -static_cast<int64_t>(totp_tolerance); offset <= static_cast<int64_t>(totp_tolerance); ++offset) {
    ...
    if (!prog.received_ports.empty() && elapsed > static_cast<int64_t>(knock_timeout_sec)) {
    ...
    for (const auto& expected : valid_sequences) {
        ...
        if (progress_idx < expected.size() && prog.received_ports.back() == expected[progress_idx]) {
            bool all_match = true;
            ...
            if (all_match) {
                event.sequence_progress = static_cast<uint32_t>(prog.received_ports.size());
                event.sequence_total = static_cast<uint32_t>(expected.size());
                if (prog.received_ports.size() >= expected.size()) {
                    ...
                    event.details = "Sequence complete -- gate opened for " + std::to_string(gate_duration_sec) + "s";
                    emit_event(event);
                    return KnockResult::GATE_OPENED;
                }
                event.result = KnockResult::SEQUENCE_PROGRESS;
                event.details = std::to_string(prog.received_ports.size()) + "/" + std::to_string(expected.size());
                emit_event(event);
...
KnockResult PortKnock::process_spa(
    const std::string& source_ip, const uint8_t* data, size_t data_len) 
{
    stats_.total_knocks.fetch_add(1);
    KnockEvent event;
    event.source_ip = source_ip;
    ...
    event.details = "SPA verified -- gate opened for " + std::to_string(gate_dur) + "s";
    emit_event(event);
...
KnockResult PortKnock::process_covert_knock(
    const std::string& source_ip, uint16_t tcp_window, uint16_t ip_id, uint8_t ttl) 
{
    std::shared_lock lock(config_mutex_);
...
void PortKnock::cleanup_expired_gates() {
    ...
    {
        std::lock_guard plock(progress_mutex_);
    ...
    {
        std::lock_guard rlock(rate_mutex_);
    ...
    {
        std::lock_guard rplock(replay_mutex_);
