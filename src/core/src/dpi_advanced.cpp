...
std::vector<uint8_t> TLSManipulator::create_fingerprinted_client_hello(
    ...
    // R7-DPI-06: Check for TLS record length overflow (max 65535 bytes)
    if (hello.size() < 5 || rl != hello.size() - 5) {
        return {};
    }
...
std::vector<uint8_t> TLSManipulator::add_tls_padding(
    ...
    // DISABLED: Appending padding after TLS record without updating
    // the record length field corrupts the TLS record.
    // TODO(R13): Implement proper TLS 1.3 padding extension (RFC 7685)
    // or send padding as a separate TCP segment.
    (void)padding_size;
...
std::vector<uint8_t> TLSManipulator::inject_grease(
    const uint8_t* data, size_t len) {
    // DISABLED: Previous implementation corrupted TLS record by writing
    // GREASE values at arbitrary offsets instead of proper RFC 8701 positions.
    // Proper GREASE is already handled in create_fake_client_hello.
    return std::vector<uint8_t>(data, data + len);
}
...
struct TrafficObfuscator::Impl {
    ...
    size_t xor_tx_offset = 0;  // R7-DPI-12: separate offset for outgoing
    size_t xor_rx_offset = 0;  // R7-DPI-12: separate offset for incoming
...
std::vector<uint8_t> TrafficObfuscator::obfuscate(const uint8_t* data, size_t len) {
    ...
    case ObfuscationMode::XOR_ROLLING: {
        result.resize(len);
        for (size_t i = 0; i < len; ++i) {
            size_t ki = (impl_->xor_tx_offset + i) % impl_->key.size();
            result[i] = data[i] ^ impl_->key[ki];
        }
        impl_->xor_tx_offset = (impl_->xor_tx_offset + len) % impl_->key.size();
        break;
    }
...
std::vector<uint8_t> TrafficObfuscator::deobfuscate(const uint8_t* data, size_t len) {
    ...
    // R7-DPI-05: Explicit cases for self-inverse and no-op modes
    case ObfuscationMode::XOR_SIMPLE: {
        // XOR_SIMPLE is self-inverse: obfuscate() doubles as deobfuscate()
        return obfuscate(data, len);
    }
    case ObfuscationMode::NONE:
        // No obfuscation — return copy of input
        return std::vector<uint8_t>(data, data + len);
    default:
        // R7-DPI-05: Unimplemented mode — return raw data and log warning
        // This prevents silent data corruption if a non-self-inverse mode
        // is accidentally used without proper deobfuscate() implementation
        return std::vector<uint8_t>(data, data + len);
    }
}
...
void TrafficObfuscator::rotate_key() {
    ...
    impl_->xor_tx_offset = 0;  // R7-DPI-12: reset TX offset on key rotation
    impl_->xor_rx_offset = 0;  // R7-DPI-12: reset RX offset on key rotation
}
...
struct AdvancedDPIBypass::Impl {
    ...
    mutable std::mutex config_mutex;  // R6-DPI-05/06/07: protects config writes
...
bool AdvancedDPIBypass::initialize(const AdvancedDPIConfig& config) {
    ...
    impl_->base_bypass->set_base_only(true);
...
std::vector<std::vector<uint8_t>> AdvancedDPIBypass::process_outgoing(
    ...
    // NOTE: inject_grease is disabled (MED-9) -- it corrupted TLS records.
    // Stat increment also disabled to avoid misleading stats.
    ...
    if (!cfg.mimicry_managed_tls &&
        ...
        if (ech_hello.size() > working_data.size()) {
            ...
            std::lock_guard<std::mutex> lock(impl_->stats_mutex);
            impl_->stats.ech_applied.fetch_add(1, std::memory_order_relaxed);
    ...
    if (!cfg.mimicry_managed_tls &&
        ...
        for (const auto& decoy_domain : cfg.base_config.decoy_sni_domains) {
            ...
            std::lock_guard<std::mutex> lock(impl_->stats_mutex);
            impl_->stats.fake_packets_injected.fetch_add(1, std::memory_order_relaxed);
        }
    }
    ...
    if (!cfg.mimicry_managed_tls &&
        ...
        std::lock_guard<std::mutex> lock(impl_->stats_mutex);
        impl_->stats.tcp_segments_split.fetch_add(segments.size(), std::memory_order_relaxed);
        ...
        if (!split_points.empty()) {
            ...
            result.insert(result.end(), segments.begin(), segments.end());
            std::lock_guard<std::mutex> lock(impl_->stats_mutex);
            impl_->stats.tls_records_split.fetch_add(1, std::memory_order_relaxed);
        } else {
    ...
    // === Padding (DISABLED: add_tls_padding is now a no-op because appending
    // padding without updating the TLS record length field corrupts records.
    // See ADV-FIX-1 / add_tls_padding for details.) ===
    ...
    if (impl_->obfuscator) {
        for (auto& segment : result) {
            ...
            std::lock_guard<std::mutex> lock(impl_->stats_mutex);
            impl_->stats.bytes_obfuscated.fetch_add(segment.size(), std::memory_order_relaxed);
        }
    }
    ...
    if (cfg.base_config.enable_timing_jitter && result.size() > 1) {
        std::lock_guard<std::mutex> lock(impl_->stats_mutex);
        impl_->stats.timing_delays_applied.fetch_add(result.size() - 1, std::memory_order_relaxed);
    }
    return result;
}
...
std::vector<uint8_t> AdvancedDPIBypass::process_incoming(const uint8_t* data, size_t len) {
    ...
    if (impl_->obfuscator) {
        ...
        std::lock_guard<std::mutex> lock(impl_->stats_mutex);
        impl_->stats.bytes_deobfuscated.fetch_add(result.size(), std::memory_order_relaxed);
        return result;
    }
    return std::vector<uint8_t>(data, data + len);
}
...
std::vector<uint8_t> DPIEvasion::apply_domain_fronting(
    ...
    if (front_domain.size() != real_domain.size()) {
        NCP_LOG_WARN("domain fronting skipped: length mismatch front=" +
                     std::to_string(front_domain.size()) + " real=" +
                     std::to_string(real_domain.size()));
        return {};
    }
