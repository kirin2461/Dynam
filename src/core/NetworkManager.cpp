...
NetworkStats NetworkManager::get_interface_stats(const std::string& name) const {
    ...
    // R10-M01: Dynamic buffer sizing with retry loop
    ULONG bufferSize = 15000;
    PIP_ADAPTER_ADDRESSES addresses = (PIP_ADAPTER_ADDRESSES)malloc(bufferSize);
    if (!addresses) return stats;
    
    DWORD ret = GetAdaptersAddresses(AF_UNSPEC, 0, nullptr, addresses, &bufferSize);
    if (ret == ERROR_BUFFER_OVERFLOW) {
        free(addresses);
        addresses = (PIP_ADAPTER_ADDRESSES)malloc(bufferSize);
        if (!addresses) return stats;
        ret = GetAdaptersAddresses(AF_UNSPEC, 0, nullptr, addresses, &bufferSize);
    }
    // Second retry if still overflow (rare case with many adapters)
    if (ret == ERROR_BUFFER_OVERFLOW) {
        free(addresses);
        addresses = (PIP_ADAPTER_ADDRESSES)malloc(bufferSize);
        if (!addresses) return stats;
