#!/bin/bash
# Dynam combat-test matrix — 2-NODE variant (runs INSIDE the client container).
# Topology: client and combined Server/DPI on ONE private bridge (172.30.0.0/24).
# The DPI lives on the server container itself (iptables -m string on INPUT,
# per-packet RST injection) — simplest possible isolated stand. Known limit:
# fake/desync packets REACH the server here (no midbox to absorb them), so
# fooling techniques that rely on packets dying mid-path behave differently
# than against a real midbox DPI (see docker/README.md, 3-node variant).
...
TARGET_HOST=${TARGET_HOST:-172.30.0.10}
PORT_BASE=11080
...
# --- testbed networking (2-node) ----------------------------------------------
# Server/DPI sits on the SAME bridge — no routing needed. Kill veth offloads
# that would reassemble split segments before the DPI string-match sees them.
ethtool -K eth0 tso off gso off gro off 2>/dev/null || true
echo "[*] 2-node stand: Server/DPI at $TARGET_HOST (shared bridge 172.30.0.0/24)" | tee -a "$LOG"
